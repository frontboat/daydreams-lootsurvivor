...
container.singleton("paradex", (container) => {
  ...
  return {
    ...
    getAccountInfo: (account: ParadexAccount) =>
      getAccountInfo(config, account),
    ...
    ) => openOrder(config, account, orderDetails),
    cancelOrder: (account: ParadexAccount, orderId: string) =>
      cancelOrder(config, account, orderId),
  };
});
...
function toQuantums(amount: BigNumber | string, precision: number): string {
...
async function debouncedAuthRefresh(
  config: ParadexConfig,
  account: ParadexAccount
) {
...
async function openOrder(
  ...
  if (price <= 0) {
    throw new Error(
      "Order failed: price must be a non-negative non-zero number."
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Order failed: ${responseData.message || response.status}`
      );
    }
...
async function main() {
  ...
  try {
    ...
    console.log(`Account:
            ...
            P&L: ${
              accountInfo.account_value && accountInfo.total_collateral
                ? accountInfo.account_value - accountInfo.total_collateral
                : "N/A"
            }
  ...
  const openOrderAction = action({
    ...
    description:
      "Open a market or limit order on Paradex. Examples: 'buy 0.1 ETH at market price' or 'buy 0.5 BTC at limit 40000'",
    schema: z.object({
      text: z
        .string()
        .describe(
          "Natural language description of the order you want to place"
        ),
    }),
    handler: async (call: { text: string }, _ctx, _agent) => {
      try {
        ...
        const orderPattern =
          /\b(buy|sell)\s+(\d+\.?\d*)\s+([a-zA-Z0-9]+)(?:\s+(?:at|@)\s+(market|limit)\s*(?:price\s*)?(?:(\d+\.?\d*))?)?/i;
        ...
        const market = availableMarkets.find(
          (m: { symbol: string }) => m.symbol === marketSymbol
        );
        ...
        if (!market) {
          return {
            ...
            message: JSON.stringify({
              text:
                `Market ${marketSymbol} is not available. Available markets:\n` +
                availableMarkets
        ...
        if (orderDetails.type === "LIMIT") {
          ...
          orderDetails.price = (
            Math.ceil(priceNum / tickSize) * tickSize
          ).toString();
        ...
        try {
          ...
          await Promise.all([debouncedAuthRefresh(config, account)]);

          return {
            success: true,
            message: JSON.stringify({
              text:
                `${orderDetails.type} order opened successfully:\n` +
  ...
  const cancelOrderAction = action({
    ...
    description:
      "Cancel an existing order. You can say something like 'cancel order 123' or 'remove order ABC'",
    schema: z.object({
          ...
          message: JSON.stringify({
            text: orders.length
              ? `Your Open Orders:\n${orders
                  .map(
                    (order: ParadexOrder) =>
                      `• ${order.market}: ${order.side} ${order.size} @ ${order.price} (${order.type})`
                  )
                  .join("\n")}`
  ...
  const listAvailableMarketsAction = action({
    ...
    schema: z.object({
      text: z
        .string()
        .describe("Natural language request to view available markets"),
    }),
    handler: async (_call, _ctx, _agent) => {
      try {
        ...
        return {
          ...
          message: {
            message: positions.length
              ? `Current positions:\n${positions
                  .map(
                    (p: { market: string; size: string; price: string }) =>
                      `${p.market}: ${p.size}${p.price ? ` @ ${p.price}` : ""}`
                  )
                  .join("\n")}`
              : "No open positions",
          },
