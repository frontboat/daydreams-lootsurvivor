import { createDreams, context } from "@daydreams-ai/core";
import type { EpisodeHooks } from "@daydreams-ai/core";
...
const supportHooks: EpisodeHooks = {
  ...
  shouldEndEpisode: (ref) => {
    ...
    const isGoodbye = content.includes("goodbye") || content.includes("have a great day");
  ...
  createEpisode: (logs, ctx) => {
    ...
    return {
      ...
      agent: {
        ...
        resolution: agentMessages[agentMessages.length - 1]?.data?.issueResolved || false,
      },
      conversation: {
  ...
  extractMetadata: (episodeData, logs, ctx) => ({
    ...
    issueType: episodeData.customer?.firstMessage?.includes("billing") ? "billing" : "general",
    responseTime: episodeData.conversation?.endTime - episodeData.conversation?.startTime,
