...
// Create Groq client for Telegram bot
...
export function formatTelegramMessage(response: any, messageType: 'text' | 'alert' | 'portfolio' = 'text'): string {
  ...
  switch (messageType) {
    ...
    case 'alert':
      if (response.type && response.message) {
        const emoji = response.priority === 'high' ? '🚨' : response.priority === 'medium' ? '⚠️' : '📢';
    ...
    default:
      ...
      if (response.accounts) {
        return `🏦 <b>Your Wallet Accounts</b>\n\n${response.accounts
          .map((acc: any) => `${acc.isActive ? '🟢' : '⚫'} <b>${acc.name}</b>\n   └ ${Object.keys(acc.balances).length} tokens`)
          .join('\n\n')}`;
      }
      
      if (response.prices) {
        return `💰 <b>Current Prices</b>\n\n${Object.entries(response.prices)
          .map(([token, price]) => `<b>${token}</b>: $${(price as number).toLocaleString()}`)
          .join('\n')}`;
      }
...
export function createWalletKeyboard(context: 'main' | 'portfolio' | 'tasks' = 'main') {
  const keyboards: Record<string, any[]> = {
    main: [
      [{ text: '💰 Portfolio', callback_data: 'portfolio' }, { text: '📊 Prices', callback_data: 'prices' }],
      [{ text: '🤖 Tasks', callback_data: 'tasks' }, { text: '🏦 Accounts', callback_data: 'accounts' }],
      [{ text: '📈 Analytics', callback_data: 'analytics' }, { text: '❓ Help', callback_data: 'help' }],
    ],
    portfolio: [
      [{ text: '💵 Balance', callback_data: 'balance' }, { text: '📊 Summary', callback_data: 'portfolio_summary' }],
      [{ text: '🔄 Simulate Trade', callback_data: 'simulate_trade' }, { text: '📈 Market', callback_data: 'market' }],
      [{ text: '🔙 Back', callback_data: 'main_menu' }],
    ],
    tasks: [
      [{ text: '➕ Create Task', callback_data: 'create_task' }, { text: '📋 List Tasks', callback_data: 'list_tasks' }],
      [{ text: '✅ Check Tasks', callback_data: 'check_tasks' }, { text: '⚙️ Manage', callback_data: 'manage_tasks' }],
      [{ text: '🔙 Back', callback_data: 'main_menu' }],
    ],
  };
...
export async function handleTelegramCallback(callbackQuery: any, userId: string) {
    ...
    handler: async (userId: string, chatId: string) => {
      ...
      return telegramWalletAgent.send({
        ...
        input: { 
          type: "text", 
          data: {
            content: "Hello! I'm new to the wallet agent. Please help me get started.",
            userId,
            platform: "telegram",
            sessionId,
          }
...
export async function simulateTelegramInteraction() {
  ...
  const startResult = await telegramCommands[0].handler(exampleUserId, exampleChatId);
  ...
  const portfolioResult = await telegramWalletAgent.send({
    ...
    input: { 
      type: "text", 
      data: {
        content: "I want to create a trading wallet with 2 ETH, 5000 USDC, and 0.1 BTC, then show me my portfolio",
  ...
  console.log("Response:", formatTelegramMessage(portfolioOutput?.data, 'portfolio'));
  ...
  const taskResult = await telegramWalletAgent.send({
    ...
    input: { 
      type: "text", 
      data: {
        content: "Create a task to alert me when BTC price goes above $70000, and another task to sell 0.05 BTC when it hits $75000",
        userId: exampleUserId,
        platform: "telegram",
        sessionId,
      }
