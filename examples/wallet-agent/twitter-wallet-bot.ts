...
export function formatTwitterMessage(
  response: any,
  messageType: "tweet" | "dm" | "reply" = "tweet"
): string {
    ...
    case "tweet":
      ...
      if (response.prices) {
        const priceList = Object.entries(response.prices)
          ...
          .map(
            ([token, price]) =>
              `${token}: $${(price as number).toLocaleString()}`
          )
          .join(" | ");
    ...
    case "dm":
      ...
      if (response.accounts) {
        return `🏦 Your Accounts:\n${response.accounts
          .map(
            (acc: any) =>
              `${acc.isActive ? "🟢" : "⚫"} ${acc.name} (${Object.keys(acc.balances).length} tokens)`
          )
          .join("\n")}`;
      }
      ...
      if (response.holdings) {
        ...
        return `📊 Portfolio Summary\n💵 Total: $${total.toLocaleString()}\n\n${response.holdings
          .map(
            (holding: any) =>
              `${holding.token}: ${holding.balance} (~$${holding.usdValue.toFixed(2)})`
          )
          .join("\n")}`;
      }
      break;
...
export function parseTwitterCommand(
  text: string,
  botUsername: string
): {
  isCommand: boolean;
  ...
  if (parsed.isCommand && parsed.command) {
    switch (parsed.command) {
      ...
      case "prices":
        ...
        message = tokens
          ? `What are current prices for ${tokens}?`
...
export async function simulateTwitterInteractions() {
  ...
  console.log(
    "Tweet Response:",
    formatTwitterMessage(mentionOutput?.data, "tweet")
  );
