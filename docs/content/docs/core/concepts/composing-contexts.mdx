...
const assistantContext = context({
  ...
  use: (state) => {
    const contexts = [
      { context: chatContext, args: { userId: state.args.userId } },
    ];

    // Add contexts based on mode
    if (state.args.mode === "voice" || state.args.mode === "multimodal") {
      contexts.push({
        context: voiceContext,
        args: { sessionId: `${state.args.userId}-voice` },
      });
    }

    if (state.args.mode === "multimodal") {
      contexts.push({
        context: visionContext,
        args: { sessionId: `${state.args.userId}-vision` },
      });
    }
...
Include contexts based on runtime conditions:

```typescript title="conditional-pattern.ts"
...
const customerServiceContext = context({
  ...
  use: (state) => [
    { context: profileContext, args: { userId: state.args.userId } },
    { context: orderContext, args: { userId: state.args.userId } },
...
```typescript title="graceful-dependencies.ts"
const enhancedContext = context({
  actions: {
    getRecommendations: action({
      handler: async (args, ctx) => {
        try {
          // Try to use ML context if available
          const mlResult = await ctx.callAction("ml.predict", {
            userId: ctx.args.userId,
          });
          return mlResult.recommendations;
        } catch (error) {
          // Fallback to simple recommendations
          return getSimpleRecommendations(ctx.args.userId);
        }
      },
    }),
  },
...
## Debugging Composed Contexts

Use the agent's logger to trace context interactions:

```typescript title="debugging-composition.ts"
const debuggableContext = context({
  type: "debuggable",
  use: (state, agent) => {
    agent.logger.info("context:compose", "Composing contexts", {
      contextType: "debuggable",
      args: state.args,
      composing: ["analytics", "profile"],
    });

    return [
      { context: analyticsContext, args: state.args },
      { context: profileContext, args: state.args },
    ];
  },

  onStep: async (ctx, agent) => {
    agent.logger.debug("context:step", "Context step", {
      availableActions: Object.keys(ctx.actions),
